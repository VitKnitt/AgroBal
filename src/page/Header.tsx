...
const Header = () => {
  ...
  const menuItems = [
    ...
    { to: "Goods", label: "Specifikace" },
    { to: "Tech", label: "Technologie" },
    { to: "Advantage", label: "Využití" },
    { to: "FAQ", label: "FAQ" },
  ];
      ...
      <ul className="hidden md:flex gap-6">
        {menuItems.map(({ to, label }) => (
          <Link
            key={to}
            activeClass="active"
            className="cursor-pointer text-primaryFont transition-colors duration-300 hover:text-secondary [&.active]:bg-tercialy [&.active]:text-secondaryFont px-2 py-1 rounded"
            to={to}
            spy={true}
            smooth={true}
            duration={500}
